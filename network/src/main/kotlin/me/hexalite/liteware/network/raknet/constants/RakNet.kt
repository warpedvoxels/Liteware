...
object RakNet {
    ...
    const val RAKNET_PROTOCOL_VERSION: Byte = 10

    const val RAKNET_MTU_PADDING: Short = 28
    const val MINIMUM_MTU_SIZE: Short = 576
    const val MAXIMUM_MTU_SIZE: Short = 1492

    const val MAXIMUM_ORDERING_CHANNELS = 16
    const val MAXIMUM_ENCAPSULATED_HEADER_SIZE: Short = 28

    const val UDP_HEADER_SIZE: Short = 8
    const val RAKNET_DATAGRAM_HEADER_SIZE: Short = 4

    const val CONNECTION_TIMEOUT_MILLISECONDS: Long = 10_000L
